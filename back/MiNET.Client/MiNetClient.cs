...
namespace MiNET.Client
{
	public class MiNetClient
	{
		...
		private static void Main(string[] args)
		{
			...
			var rnd = new Random();
			var name = "not a bot 2892";
			//"pidr " + rnd.Next(0, 10000).ToString("####");
			Console.WriteLine(name);

			var senderEndpoint = new IPEndPoint(Dns.GetHostEntry("play.greenwix.fun").AddressList[0], 20112);

			var client = new MiNetClient(senderEndpoint, name, new DedicatedThreadPool(new DedicatedThreadPoolSettings(Environment.ProcessorCount)));
			...
			client._serverEndpoint = senderEndpoint;
			...
			var password = name.Replace(" ", "_");
			Task.Run(BotHelpers.DoWaitForSpawn(client))
				//.ContinueWith(t => Task.Delay(1000).Wait())
				//.СontinueWith(t => client.SendChat($"./r {password} {password}"))
				//.ContinueWith(t => Task.Delay(2000).Wait())
				//.ContinueWith(t => client.SendChat($"./l {password}"))
			...
			while(true)
			{
				...
				switch (ag[0].ToLower())
				{
					...
					case "chat":
					case "c":
						client.SendChat(string.Join(" ", ag.Skip(1).ToArray()));
						break;
					case "circle":
						Console.WriteLine("going in circles!");
						Circles(client);
						break;

					case "crit":
						Console.WriteLine("CRIT!");
						CritAttack(client, int.Parse(ag[1]));
						break;
					case "follow":
					case "f":
						if(args.Length == 1)
						...
						var username = string.Join(" ", ag.Skip(1).ToArray());
						var player = client.Players.FirstOrDefault(l => l.Username?.ToLower() == username.ToLower());
						if(player == null)
						{
							Console.WriteLine("no such player " + username);
							continue;
						}

						Console.WriteLine($"Following player {player.Username}");
						client.FollowedPlayer = player;
		...
		private static async void CritAttack(MiNetClient client, int id)
		{
			for(var i = 0; i < 20 * 30 * 100; i++)
			{
				var anim = McpeAnimate.CreateObject();
				anim.actionId = id;
				anim.runtimeEntityId = client.EntityId;
				client.SendPackage(anim);

			}
		}

		private async Task<int> MoveTo(PlayerLocation pos)
		{
			Vector3 originalPosition = CurrentLocation.ToVector3();
			Vector3 targetPosition = pos.ToVector3();

			PlayerLocation lookAtPos = BotHelpers.LookAt(originalPosition + new Vector3(0, 1.62f, 0), targetPosition);
			...
			while (true)
			{
				if (Math.Abs((targetPosition - CurrentLocation.ToVector3()).Length()) > stepLen)
				{
					float lenLeft = Math.Abs((CurrentLocation.ToVector3() - targetPosition).Length());
					weight = Math.Abs((float) ((lenLeft - stepLen) / lenght));

					CurrentLocation = new PlayerLocation(Vector3.Lerp(originalPosition, targetPosition, 1 - weight));
					...
					movePlayerPacket.y = CurrentLocation.Y + 1.62f;
				...
				{
					CurrentLocation = new PlayerLocation(targetPosition);
		...
		private static async void Circles(MiNetClient client)
		{
			var i = 0;
			var r = 10;
			var startingPoint = (PlayerLocation) client.CurrentLocation.Clone();
			var startX = Math.Sin(i) * r;
			var startZ = Math.Cos(i) * r;
			await client.MoveTo(startingPoint + new Vector3((float) startX, 0f, (float)startZ));

			while (true)
			{
				var x = Math.Sin(i) * r;
				var z = Math.Cos(i) * r;
				await client.MoveTo(startingPoint + new Vector3((float) x, 0f, (float) z));
		...
		private void ProcessDatagrams(object state)
		{
			while (true)
			{
				...
				try
				{
					...
					if (receiveBytes.Length != 0 && senderEndpoint.Equals(_serverEndpoint))
					{
		...
		private async void Transfer(McpeTransfer transfer)
		{
			WS.Send(transfer);
			/*Console.WriteLine("========");
			Console.WriteLine("Transfering to " + transfer.serverAddress + ":" + transfer.port);
			Console.WriteLine("========");*/

			/*StopClient();

			_serverEndpoint = new IPEndPoint(Dns.GetHostEntry(transfer.serverAddress).AddressList[0], transfer.port);
			HaveServer = false;
		...
		private void HandlePackage(Package message)
		{
			...
			else if (typeof(McpeBossEvent) == message.GetType())
			{
			}
			...
			else if (typeof(McpeTransfer) == message.GetType())
			{
				Transfer((McpeTransfer) message);
			}
		...
		{
			WS.Send(message);
			if (FollowedPlayer != null && message.runtimeEntityId == FollowedPlayer.EntityId)
			{
				CurrentLocation = (PlayerLocation)message.position.Clone();
				SendMcpeMovePlayer();
			}
		...
		private void OnMcpePlayerList(McpePlayerList message)
		{
			...
			var arr = new JArray();
			jo["pk"] = "PlayerList";
			jo["type"] = message.records is PlayerAddRecords ? 1 : 0;
			jo["players"] = arr;
			foreach (var playerRecord in message.records)
			{
				if(message.records is PlayerAddRecords)
				{
					playerRecord.Username = playerRecord.DisplayName;
					Players.Add(playerRecord);

					var bytes = playerRecord.Skin.Texture;
					int width = 64;
					...
					using(var g = Graphics.FromImage(bmp))
					{
						...
						g.DrawImage(bitmap.Clone(new Rectangle(8, 8, 8, 8), PixelFormat.DontCare), 0, 0, 32, 32);
					}
					bmp.Save(ms, ImageFormat.Png);

					arr.Add(new JObject
					{
						{ "uuid", playerRecord.ClientUuid.ToString() },
						{ "username", playerRecord.Username },
						{ "entityId", playerRecord.EntityId },
						{ "skin", Convert.ToBase64String(ms.ToArray()) },
					});
				} else if (message.records is PlayerRemoveRecords)
				{
					foreach(var p in Players.ToArray())
					{
						if(p.ClientUuid == playerRecord.ClientUuid)
						{
							Players.Remove(p);
							break;
						}
			...
			Console.Title = $"Players {Players.Count}";
...
");
		...
		{
			Log.Warn("Play status = " + message.status);
		...
		private void OnMcpeText(McpeText message)
		{
			...
			//Log.Error("Text: " + message.message);
		...
		private void OnMcpeAddPlayer(Package message)
		{
			...
			jo["entityTypeName"] = EntityType.Player.ToString();
		...
		private void OnMcpeAddEntity(McpeAddEntity message)
		{
			...
			var jo = JObject.Parse(JsonConvert.SerializeObject(message, jsonSerializerSettings));
			jo["entityTypeName"] = ((EntityType) typeBytes[0]).ToString();
			jo["pk"] = message.GetType().Name;
			WS.Send(jo.ToString());
			...
			if (_chunks.TryAdd(new Tuple<int, int>(msg.chunkX, msg.chunkZ), true))
			{
				...
				try
				{
					...
					if (chunk != null)
					{
						...
						jo["x"] = chunk.x;
						jo["z"] = chunk.z;
						jo["data"] = Convert.ToBase64String(blocks);
						WS.Send(jo.ToString());
