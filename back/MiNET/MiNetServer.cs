...
namespace MiNET
{
	public class MiNetServer
	{
		...
		private void ProcessDatagrams(object state)
		{
			...
			while (true)
			{
				...
				try
				{
					...
					if(ProxyFrom == null && senderEndpoint.Address.Equals(IPAddress.Loopback)) ProxyFrom = senderEndpoint;

					if (senderEndpoint.Equals(ProxyTo))
					{
						Console.WriteLine("RESENDING BYTES TO " + ProxyFrom);
						var msgId = receiveBytes[0];
						if (msgId <= (byte) DefaultMessageIdTypes.ID_USER_PACKET_ENUM)
						{

							var message = PackageFactory.CreatePackage(msgId, receiveBytes, "raknet");
							Log.Error("<<< RAKNET: " + message);
							if (message is OpenConnectionReply1 reply1)
							{
								reply1.mtuSize = 1464;
								Console.WriteLine(reply1.mtuSize);
								Console.WriteLine(reply1.offlineMessageDataId.Length);
								Console.WriteLine(reply1.serverHasSecurity);
								SendData(message.Encode(), ProxyFrom);
							}
							else
							{
								SendData(receiveBytes, ProxyFrom);
							}
					...
					} else
					{
						...
						if (msgId <= (byte) DefaultMessageIdTypes.ID_USER_PACKET_ENUM)
						{
							...
							var message = PackageFactory.CreatePackage(msgId, receiveBytes, "raknet");
							Log.Error(">>> RAKNET: " + message);
							if (message is OpenConnectionRequest1 reply1)
							{
								Console.WriteLine(reply1.mtuSize);
								Console.WriteLine(reply1.offlineMessageDataId.Length);
								Console.WriteLine(reply1.raknetProtocolVersion);
								SendData(receiveBytes, ProxyTo);
							}
							else
							{
								SendData(receiveBytes, ProxyTo);
		...
		public IPEndPoint ProxyTo = new IPEndPoint(Dns.GetHostEntry("play.greenwix.fun").AddressList[0], 19132);
		public IPEndPoint ProxyFrom = null;
				...
				*/
				...
				if (!header.isACK && !header.isNAK && header.isValid)
				{
					...
					try
					{
						...
						Log.Error(">>> MCPE: " + receiveBytes.Length);
					}
					catch (Exception e)
					{
